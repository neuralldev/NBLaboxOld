...
// now seach for specific numericable information in it such as login form
    ...
    public function getPage($url, $cookie, $referer='') {
         $getparams = array(
            'url' => $url,
            'host' => '',
            'header' => '',
        ...
        log::add('NBLabox', 'debug', __METHOD__ . ' get page '.$url.' referer='.$referer);        
        $getpage = new CurlRequest($getparams);
        return  $getpage->exec();        
    }

    public function postPage($url, $postparams, $cookie, $referer ='') {
         $getparams = array(
            'url' => $url,
            'host' => '',
            'header' => '',
        ...
        log::add('NBLabox', 'debug', __METHOD__ . ' post page '.$url.' referer='.$referer);        
        $getpage = new CurlRequest($getparams);
        return $getpage->exec();
    }
...
//         get basic url from current request
        log::add('NBLabox', 'debug', __METHOD__ . ' attempt login '.  'http://' . $urlitems['host']);
        $result = $this->postPage('http://' . $urlitems['host'] . '/goform/login',  'loginUsername=' . $login . '&loginPassword=' . urlencode($password), $this->param['cookiejar'], 'http://' . $urlitems['host'].'/config.html');             
...
//        log::add('NBLabox', 'debug', __METHOD__ . ' logout '.json_encode($params, true));
 ...
 //       log::add('NBLabox', 'debug', __METHOD__ . ' reset modem attempt '.json_encode($result, true));        
        $result = $this->postPage('http://' . $urlitems['host'] . '/goform/WebUiOnlyReboot',  '', $this->param['cookiejar'], 'http://' . $urlitems['host'] .'/config.html');            
